...
-- NOTE: The "Prospecting" and "Engaging" statuses represent intermediate phases in the sales process,
-- so they are not included in the success rate calculation.
...
SELECT st.regional_office AS teams_regional_office,
       ...
       ROUND(
         ...
         GREATEST(SUM(CASE WHEN sp.deal_stage IN ('Won', 'Lost') THEN 1 ELSE 0 END), 1), 2
       ) AS success_rate_pct
FROM sales_pipeline sp
JOIN sales_teams st ON sp.agent_id = st.agent_id
GROUP BY st.regional_office, st.manager
...
success_rates AS (
    SELECT teams_regional_office,
           ...
           CAST(won_deals AS FLOAT) / NULLIF(won_lost_deals, 0) AS agents_success_rate,
           SUM(won_deals) OVER (PARTITION BY teams_regional_office, teams_manager) / 
           NULLIF(SUM(won_lost_deals) OVER (PARTITION BY teams_regional_office, teams_manager), 0) AS teams_success_rate
    FROM sales_deals
)
SELECT teams_regional_office,
       teams_manager,
       sales_agent,
       ROUND(CAST(agents_success_rate * 100 AS numeric), 2) AS agents_success_rate_pct,
       ROUND(CAST(teams_success_rate * 100 AS numeric), 2) AS team_success_rate_pct,
       CASE WHEN agents_success_rate > teams_success_rate THEN 'Above Team Average' 
            ELSE 'Below Team Average' END AS success_rate_description
FROM success_rates
...
SELECT quarter,
       won_opportunities,
       ROUND(
           CASE WHEN prev_won_opportunities IS NOT NULL AND prev_won_opportunities != 0 
                THEN ((won_opportunities::numeric - prev_won_opportunities::numeric) / prev_won_opportunities::numeric) * 100 
                ELSE NULL 
           END, 2) AS qoq_won_opportunities_growth_pct,
       sales_revenue,
       ROUND(
           CASE WHEN prev_sales_revenue IS NOT NULL AND prev_sales_revenue != 0 
                THEN ((sales_revenue::numeric - prev_sales_revenue::numeric) / prev_sales_revenue::numeric) * 100 
                ELSE NULL 
           END, 2) AS qoq_sales_revenue_growth_pct
FROM qoq_trends;
...
qoq_success_rate AS (
    SELECT *,
           ...
           LAG((won_deals::numeric / NULLIF(won_lost_deals, 0))) OVER (ORDER BY quarter ASC) AS prev_success_rate
    FROM quarter_sales_deals
)
SELECT quarter,
       ROUND(success_rate * 100, 2) AS success_rate_pct,
       ROUND(
           CASE WHEN prev_success_rate IS NOT NULL AND prev_success_rate != 0
                THEN ((success_rate - prev_success_rate) / prev_success_rate) * 100
                ELSE NULL
           END, 2) AS qoq_success_rate_growth_pct
...
SELECT p.product_name,
       ROUND(
           (SUM(CASE WHEN sp.deal_stage = 'Won' THEN 1 ELSE 0 END)::numeric /
            NULLIF(SUM(CASE WHEN sp.deal_stage IN ('Won', 'Lost') THEN 1 ELSE 0 END), 0)) * 100, 
           2
       ) AS success_rate_pct
FROM sales_pipeline sp
...
SELECT product_name,
       sales_revenue,
       ROUND(CAST((sales_revenue / SUM(sales_revenue) OVER ()) AS numeric), 2) * 100 AS revenue_pct
...
SELECT a.sector,
       ...
       ROUND(
           SUM(CASE WHEN sp.deal_stage = 'Won' THEN 1 ELSE 0 END)::numeric / 
           NULLIF(SUM(CASE WHEN sp.deal_stage = 'Lost' THEN 1 ELSE 0 END), 0), 2
       ) AS win_loss_ratio
FROM sales_pipeline sp
JOIN accounts a ON sp.account_id = a.account_id
GROUP BY a.sector
